...
for i in clust:
    ...
    r = []
    for j in range(1901, 2003):
        temp = data[data['Years'] == j]
        rain = list(temp['Rainfall'])
        r.append(np.median(rain))
    d[i] = r
columns = ['Year'] + clust
df = pd.DataFrame(d, columns=columns)
x = list(range(1901, 2003))
...
plt.xticks(range(1901, 2003), rotation=90)
